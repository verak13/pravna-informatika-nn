...
with open('pickles-for-testing/tokenizer.pkl', 'rb') as pickle_handle:
    tokenizer_inputs = pickle.load(pickle_handle)

# load dictionaries
with open('pickles-for-testing/word2idx_inputs.pkl', 'rb') as pickle_handle:
    word2idx_inputs = pickle.load(pickle_handle)
with open('pickles-for-testing/word2idx_outputs.pkl', 'rb') as pickle_handle:
...
from_to = [3000, 4000, 100]
model_num_list = [str(x) for x in range(*from_to)]
model_list =[ 'models/s2s-25-vanilla/s2s-' + x.zfill(8) + '.h5' for x in model_num_list]
...
for k, test_text in enumerate(input_texts):
    ...
    # of = open('results/s2s-25-vanilla/s2s-25-3000-3900-test/' + str(k) + '.txt', 'w', encoding='utf-8')
